...
-1 # min year for sdreport outputs (-1 for styr); #_1901
-2 # max year for sdreport outputs (-1 for endyr+1; -2 for endyr+Nforecastyrs); #_2026
...
5 # F_std_reporting_units: 0=skip; 1=exploitation(Bio); 2=exploitation(Num); 3=sum(Apical_F's); 4=mean F for range of ages (numbers weighted); 5=unweighted mean F for range of ages
 3 6 # min and max age over which mean F will be calculated, with F=Z-M
...
-1 # random number seed for bootstrap data (-1 to use long(time) as seed): # 1742541294
